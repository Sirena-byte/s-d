...
{
	$sql = "SELECT a.title, a.text, a.file, au.time_change, u.name AS user_name, u.email, o.name AS organization, d.name AS department, p.address, us.name AS status_user FROM `application` a JOIN `application_user`au ON a.id_app = au.id_app JOIN `users` u ON u.id_user = au.id_user JOIN `places_of_work` p ON p.id_place = u.id_place_of_work JOIN `organizations` o ON o.id_organization = p.id_organization JOIN `departments` d ON d.id_department = p.id_department JOIN `user_statuses` us ON us.id_status = au.id_user_status WHERE a.id_app = :id";
